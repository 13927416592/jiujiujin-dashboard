...
function getTrafficTab(trafficTabs: Record<string, any>, key: string): TabLike {
...
function parseMiniProgramData(mp: any) {
  ...
  return { id: mp.id, name: mp.name, tabs: result };
}
...
export function parseAlipayRaw(rawData: any, date?: string) {
  ...
  const lifeAccountMetrics = parseLifeAccountMetrics(lifeAccount.bodyText || '');
  const fanGroupMetrics = parseTrafficFanGroupMetrics(fanGroup.bodyText || '');
