...
### 日期范围规则（首次基线 + 每日追加）
美团与支付宝均用环境变量控制：
- 每日定时任务默认 `*_EXPORT_DAYS=1`，取"昨天"，存每日快照。
- 首次回填：美团 `MEITUAN_EXPORT_DAYS=30`（报表导出每天一行，自动拆 30 条）；
  支付宝 `ALIPAY_EXPORT_DAYS=7`（各数据页是周期汇总而非每日明细，只点"7日/近7日"取近7日为一条基线，不做 30 天逐天抓取）。
- 支付宝各页日期控件差异（`alipay.ts#applyDateRange` 已按文案兼容）：
  - 经营总览/交易分析/流量分析(5 Tab)/粉丝群：`1日 | 7日 | 30日 | 自然月`
  - 生活号+分析：`1日 | 近7日 | 近30日`
  - 用户分析：只有单日日历，无 7日 切换，两种模式都取其默认最近一天
